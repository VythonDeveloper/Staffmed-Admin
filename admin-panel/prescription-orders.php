...
if(isset($_POST['isPaid'])){
    $isPaid = $_POST['isPaid'];
    $presOdId = $_POST['presOdId'];
    $conn->query("Update prescription_orders set isPaid = '$isPaid' where id = '$presOdId'");
}
if(isset($_POST['orderStatus'])){
    $orderStatus = $_POST['orderStatus'];
    $presOdId = $_POST['presOdId'];
    $conn->query("Update prescription_orders set status = '$orderStatus' where id = '$presOdId'");
}
$pageNo = 0;
$searchKey = '';
...
$presOrdersData = getPrescriptionOrdersList($pageNo, $searchKey);
include "header.php";
?>
...
<div class="relative overflow-x-auto shadow-md sm:rounded-lg">
    ...
    <table class="w-full text-sm text-left text-gray-500 light:text-gray-400" id="dataTable">
        ...
        <tbody>
            <?php foreach($presOrdersData as $value){ ?>
                <form method="POST" onsubmit="return confirm('Are you sure to make following changes?');">
                    <input type="hidden" value="<?php echo $value['id'];?>" name="presOdId">
                    <tr class="bg-white border-b light:bg-gray-800 light:border-gray-700 hover:bg-gray-50 light:hover:bg-gray-600">
                        ...
                        <td class="px-3 py-4 text-right">
                            <div class="inline-flex rounded-md shadow-sm" role="group">
                                <a href="prescription-details.php?presOdId=<?php echo $value['id'];?>" target="_blank" class="cursor-pointer px-3 py-2 text-xs font-medium text-center inline-flex items-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
