...
if($mandatoryVal){
    ...
    if($processStatus["error"] == false && isAuthorized()){
        $res = $conn->query("Select * from cart where userId = '$userId' and productId = '$productId'");
        if($res->num_rows == 0){
            ...
            if($conn->affected_rows > 0){
                $processStatus["error"] = false;
                $processStatus["message"] = "Product added to Cart";
            } else{
            ...
            $processStatus["error"] = true;
            $processStatus["message"] = "Product already in cart. Continue to checkout";
        }
    } else{
        // Error Part
        $processStatus["error"] = true;
