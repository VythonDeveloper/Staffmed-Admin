...
if(isset($_POST['from']) && $_POST['from'] == "Update-Medicine"){
    $medId = getSafeValue($_POST['medId']);
    $oldProductImg = getSafeValue($_POST['oldProductImg']);
    ...
    if($isImageAttached == "Yes"){
        ...
        if(move_uploaded_file($_FILES['productImg']['tmp_name'],"./assets/".$fname)){
            ...
            $oldProductImg = explode('/', $oldProductImg);
            try{
                unlink("./assets/".end($oldProductImg));
            } catch(Exception $e) {
            }
            $goodToGo = true;
        }
    ...
    if($goodToGo){
        ...
        if($conn->affected_rows>0){
            echo json_encode(array("error" => false, "message" => "Medicine Details Updated."));
        }
        else{
